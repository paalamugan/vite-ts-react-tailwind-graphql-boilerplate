...
type BaseDateType = string | Date | number;
...
// relative time
class DateIntl<D extends BaseDateType> {
  ...
  constructor({ locale, dateFormat, dateTimeFormat, transformer, fallback }: IDateOptions<D>) {
    this.locale = locale;
  ...
  public formatDate(value: D): string {
    ...
    return Intl.DateTimeFormat(this.locale, this.dateFormat).format(this.toDate(value));
  }

  public formatDateTime(value: D): string {
    if (!this.isValid(value)) return this.fallback;

    return Intl.DateTimeFormat(this.locale, this.dateTimeFormat).format(this.toDate(value));
  }
  ...
  public isValid(value: unknown): boolean {
    if (!value || isNaN(Date.parse((value as string).toString()))) {
      return false;
    }
  ...
  private toInput(value: unknown): string {
    ...
    return this.transformer.convert(date);
  }
}
